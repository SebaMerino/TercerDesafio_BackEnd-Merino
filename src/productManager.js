    ...
    addProduct = async(title,description,price,thumbnail,code,stock) => {
     ...
     updateProduct = async(id,title,description,price,thumbnail,code,stock) => {
        if(!id|| !title || !description || !price || !thumbnail|| !code||!stock){
          console.error("INGRESE TODOS LOS DATOS DEL PRODUCTO PARA SER ACTUALIZADO")
          return 
        }
        else{
            ...
            if(codigorepetido){
                 console.error("EL CODIGO DEL PRODUCTO QUE DESEA ACTUALIZAR ESTA REPETIDO")
                 return
            }
            else{
      ...
      deleteProduct = async(id) => {
        const allproducts= await this.getProducts()
        const productosEncontrado= allproducts.filter(elemento=>elemento.id!==id)
       await fs.promises.writeFile(this.path,JSON.stringify(productosEncontrado,null,2))
      }
    getProductbyId = async(id) => {
        const allproducts= await this.getProducts()
       const encontrado= allproducts.find(element=>element.id===id)
       return encontrado
    }
}
